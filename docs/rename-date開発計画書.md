...
指定されたフォルダ配下を再帰的に走査、またはドラッグ&ドロップされた個別ファイルを対象に、正規表現パターン（例: (YYYY.M.D) 形式）にマッチするファイル名を指定の日付表現（既定: ISO8601 簡易表現 `YYYYMMDD`）へ一括変更する GUI アプリケーションを開発する。置換対象パターンと置換後表現は「プリセット」として JSON に永続化し、GUI 上で選択・追加・編集・削除できる。
...
- **可変正規表現 & 出力テンプレート**: 置換対象パターン（年・月・日の 3 キャプチャグループ契約）と置換後の出力テンプレートを GUI 上でプリセットとして選択・編集可能。プリセットは JSON で永続化される。
...
| パターン管理     | 置換対象パターンと出力テンプレートの組を「プリセット」として JSON に永続化。GUI で選択・追加・編集・削除可能 |
| パターン保存先   | `%APPDATA%\rename-date\config\patterns.json`（UTF-8）。初回起動時は既定プリセット1件を生成 |
...
| - PatternService  : パターンプリセットの読込/保存/検証(JSON) |
+-----------------------------------------------------------+
                     | (データ共有)
                     v
+-----------------------------------------------------------+
...
| - PatternEntry     : パターンプリセット（正規表現+出力テンプレート） |
+-----------------------------------------------------------+
```
...
│       │   └── pattern_entry.py      # PatternEntry（パターンプリセット）
│       │
│       ├── services/                 # コアビジネスロジック
│       │   ├── __init__.py
│       │   ├── scanner_service.py    # 対象収集・正規表現マッチング・衝突解決
...
1. **設定エリア (ConfigFrame)**
   ...
   - 正規表現パターンは `ttk.Combobox` でパターンプリセット（名前・置換対象パターン・出力テンプレートの組）から選択。既定は名前「既定 (YYYY.M.D)」、パターン `\((\d{4})\.(\d{1,2})\.(\d{1,2})\)`、出力テンプレート `{Y}{M}{D}`
   - 「プリセットを保存/上書き」「プリセットを削除」「新規プリセット」ボタンを配置し、`PatternService` 経由で JSON へ即時永続化する
   - 「プレビュー更新」ボタン
...
- 妥当な場合は選択中のプリセットの `output_template` に従い、`{Y}`=4桁ゼロ埋め年・`{M}`/`{D}`=2桁ゼロ埋め月日を埋め込んで生成する。括弧は結果に含めない。
...
#### **E. パターンプリセット管理 (PatternService)**

- 起動時に `%APPDATA%\rename-date\config\patterns.json` を読み込む。存在しない・パース失敗・スキーマ不正の場合は既定プリセット 1 件（名前「既定 (YYYY.M.D)」、パターン `\((\d{4})\.(\d{1,2})\.(\d{1,2})\)`、出力テンプレート `{Y}{M}{D}`）のみを持つファイルを新規生成する。
- 保存時は一時ファイルに書き込んだ後 `os.replace()` でアトミックに置き換える（リネーム対象ファイルの操作とは無関係の内部処理）。
- 各プリセットは保存前に ScannerService と同じ検証（正規表現コンパイル可否・キャプチャグループ数=3・出力テンプレートのプレースホルダ/禁止文字チェック）を行い、不正なら保存を拒否してエラー表示する。
...
11. **パターンプリセットの不正**: 保存時の検証（正規表現・出力テンプレート）に失敗した場合は保存を拒否しエラーダイアログで通知する。patterns.json 自体が壊れている場合は既定プリセットにフォールバックする。
...
| テストファイル    | 主な観点                                                                                                                                                                                                                             |
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `test_scanner.py` | 基本変換 / ゼロ埋め / 1 ファイル内の複数マッチ / 拡張子の非対象 / 不正日付 / 隠しファイル・除外フォルダ / シンボリックリンク / 既存ファイルとの衝突 / 同一バッチ内の衝突 / 不正な正規表現とグループ数不一致 / 変更なしファイルの除外 / 出力テンプレートの可変化（区切り文字違い）/ 個別ファイルリストからのプレビュー生成 / フォルダ・ファイル混在ドロップの重複除去 |
...
| `test_pattern_service.py` | JSON の読込/保存 / 初回起動時の既定プリセット生成 / 不正テンプレート（プレースホルダ欠落・禁止文字）の保存拒否 / 壊れた JSON からの既定プリセットフォールバック / 最後の1件削除拒否 |
...
9. 既定プリセットのほかに新規プリセット（別区切り文字の出力テンプレート）を作成・保存し、アプリ再起動後も選択肢に残っていること
